...
bool MainOperDirecitve::isValid(QStringList &errors) const {
    ...
    if (m_directive_.command_lines.at(0).operation.trimmed().isEmpty()) {
        errors.append("Не указана операция в 1 строке директивы!");
        res = false;
    }
...
void MainOperDirecitve::start() {
    ...
    for (int num_line = 0; num_line < m_directive_.command_lines.count(); ++num_line) {
        QString line = m_directive_.command_lines[num_line].operation.trimmed();
        if (!line.isEmpty()) {
            operation_.append(" " + line);
        }
    }
...
void MainOperDirecitve::onWindowResult(const WindowResultData& result) {
    ...
    pris_message.message = operation_;
